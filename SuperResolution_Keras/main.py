...
#%% MODEL
# mainModel
def Model_Block(scale = 2, num_filters = 64, num_res_blocks = 8, res_block_scaling = None, model_name = None, x_in = None): #origin (4, 64, 16, None)
    if x_in is None:
...
#%% 
x_in, x_64,  m_branch = Model_Block() # 32-64
_,    x_128, _        = Model_Block(x_in = m_branch) # 64-128
...
for epoch in range(epochs):
    ...
    for step, (batch_in, batch_mid, batch_out) in enumerate(dataloader):
        loss_out = model_all.train_on_batch(batch_in, [batch_mid, batch_out])
